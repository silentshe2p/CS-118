...
int RoutingTable::maskLength(uint32_t mask) const
{
   int length;
   for (length = 0; mask; mask >>=1) {
     length += mask & 1;
   }
   return length;
}
...
RoutingTable::lookup(uint32_t ip) const
{
  ...
  for (auto const& it : m_entries) {
    // Check destination while comparing prefix length
    if (((ip & it.mask) == (it.dest & it.mask)) && (maskLength(it.mask) > longestSoFar)) {
      if (!found)
        found = true;
      longestSoFar = maskLength(it.mask);
      rte = it;
    }
  }  
  // No entry found
